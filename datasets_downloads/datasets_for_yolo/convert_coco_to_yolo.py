...
def process_images_and_annotations(base_coco_folder, output_folder):
    """
    ...
    - output_folder (str): The base folder path where the converted dataset should be saved.
    ...
    for img, (image_id, img_file_name) in enumerate(image_id_to_file_name.items()):
        ...
        output_txt_file = os.path.join(output_folder, 'labels', img_file_name.replace('.jpg', '.txt'))
        ...
        dest_img_path = os.path.join(output_folder, 'images', img_file_name)
        shutil.copy(source_img_path, dest_img_path)


if __name__ == "__main__":
    """ Usage: python convert_coco_to_yolo.py """

    # Parameters; Change paths to desired directories
    base_coco_folder = "/path/to/UCD/coco/dataset"
    output_folder = "/output/path/to/UCD/yolo/dataset"
    os.makedirs(output_folder, exist_ok=True)

    # Set output folder for YOLO format annotations
    train_images_folder = os.path.join(output_folder, "images")
    txt_train_folder = os.path.join(output_folder, "labels")
    try:
        os.makedirs(train_images_folder, exist_ok=True)  # Allow re-creation if already exists
        os.makedirs(txt_train_folder, exist_ok=True)  # Allow re-creation if already exists
    except FileExistsError:
        # If directories already exist, you can decide to print a warning or pass
        print("Warning: Output directories already exist and will be used.")

    print("Creating YOLO format dataset including negative frames...")
    process_images_and_annotations(base_coco_folder, output_folder)
    print("DONE.")
